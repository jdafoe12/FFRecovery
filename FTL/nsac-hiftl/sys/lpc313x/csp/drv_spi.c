...
void SPI_configure(SPI_CNFG * spi_cnfg) {
  ...
  /*If Slave mode*/
  if (spi_cnfg->MS_Mode)
    SPI_SLAVE_ENABLE = 0; /*Clear Slave Enable*/
}
/*************************************************************************
...
void Slave_configure(SLAVE_CNFG * Slave_cnfg) {
  Int32U * SlaveSetting1 = (Int32U *) (&SPI_SLV0_SETTINGS1
      + (2 * Slave_cnfg->Slave));
  Int32U * SlaveSetting2 = (Int32U *) (&SPI_SLV0_SETTINGS2
      + (2 * Slave_cnfg->Slave));
  /*configure clock Divisors*/
  Int32U clock_ratio = (SPI_MAIN_CLOCK + Slave_cnfg->Clock / 2)
      / Slave_cnfg->Clock;
  Int32U div2, tmp = 0;

  /*Corect Clock Ratio*/
  if (SPI_MIN_CLK_R > clock_ratio)
    clock_ratio = SPI_MIN_CLK_R;
  if (SPI_MAX_CLK_R < clock_ratio)
    clock_ratio = SPI_MAX_CLK_R;
  /*Set Clock Divisors*/
...
Int32U SPI_read_fifo(SLAVE_ID Slave, void *buffer, Int32U Words) {
  ...
  Int16U *data16 = NULL;
  Int8U *data8 = NULL;
  volatile __spi_slv_settings2_bits * SlaveSetting2 =
      (__spi_slv_settings2_bits *) (&SPI_SLV0_SETTINGS2 + (2 * Slave));
