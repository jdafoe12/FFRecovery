...
void MSC_Init()
{
   ...
   //jsjpenn
   //MSC_BlockCount = ONFM_Capacity();//原逻辑
   //多加了20个扇区，即5个LBA地址
   //由于预留的存MAPI的LEB块数有限，所以不能添加过多的LBA
   //原来g_LBAnum=240345，LBA为0~240344，现在LBA为0~240348
   MSC_BlockCount = ONFM_Capacity()+20;
}
...
void MSC_MemoryWrite(void)
{
   ...
   if (BulkLen%MSC_BlockSize == 0)
   {
      ...
      ut_list[ut_push].length = SECTOR_PER_MPP;//4
