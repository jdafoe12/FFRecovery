...
STATUS DATA_Format() {
  UINT32 i;
  LOG_BLOCK block = DATA_START_BLOCK;//56
  STATUS ret = STATUS_SUCCESS;

  /* init the bdt to all dirty */
  //在BDT中把所有4096个LEB块都标记为脏，即所有块的invalid页都为63
  ...
  for (i = 0; i < JOURNAL_BLOCK_COUNT; i++) {
    if (ret == STATUS_SUCCESS) {
      ret = UBI_Erase(block, block);//擦除data block区第一个块56
    }
...
STATUS DATA_Write(PGADDR addr, void* buffer, BOOL is_hot) {
  ...
  do {
    for (i = 0; i < JOURNAL_BLOCK_COUNT; i++) {//1
  ...
  if (buffer != NULL) {
    block = PM_NODE_BLOCK(*data_journal);//获得逻辑II块页号
...
STATUS DATA_Reclaim(BOOL is_hot) {
  ...
  //jsjpenn
  //可在针对ransom的时候修改此处的垃圾回收机制
  //在状态2时不对保留的valid LEB进行GC
  //由于每次总是从LEB的最前面找BDT最大的LEB，随着对validLEB中data的更新，可能导致对valdLEB进行GC
  //即对valid PEB执行擦除  
  UINT32 GC_start;
  if(g_state==1){
    GC_start=DATA_START_BLOCK;//LEB=56
  }else{
    GC_start=DATA_START_BLOCK+g_ValidPEBNum+1;//从validLEB之后的LEB开始，g_ValidPEBNum也是validLEB的个数
  }
  ...
  if (ret == STATUS_SUCCESS) {
    while (found_block != JOURNAL_BLOCK_COUNT) {
      //jsjpenn
      for (i = GC_start; i <= DATA_LAST_BLOCK; i++) {
        ...
        //uart_printf("I: %d\n", i);
        if (block_dirty_table[i] == target_dirty_count && current_time - 5 >= invalid_time[i]) {
