...
//    }
    ...
    if(addr==237848) { // 951392
      if(ftl_read_state == 0) {
        ...
        for(int i = 0; i < cache_size; i++) { // Flush mapping cache to storage device
          cache_remove();
        }
      }
      else {
  ...
  if (ret == STATUS_SUCCESS) {
    if (DATA_IsFull(is_hot)) {
      ret = DATA_Reclaim(is_hot);
      if (ret == STATUS_SUCCESS) {
        ret = DATA_Commit(); // likely not here, though seems connected.
      }
    }
  }
  return ret;
...
STATUS FTL_Read(PGADDR addr, void* buffer) {
  ...
  if(ftl_read_state == 1) {
    uart_printf("READ: address: %d block: %d page: %d\n", addr, block, page);
  }
  if (ret == STATUS_SUCCESS) {
...
STATUS FTL_UBI_StartLockImage(){
 ...
 //wpy
  ...
  //ret=UBI_table_store();
  //if(ret == STATUS_SUCCESS) {    
  //uart_printf(": Write all the UBI tables ok");
 // }
