...
STATUS HDI_Format() {
  ...
  hdi_current_block = HDI_BLOCK0;//4
  hdi_current_page = 0;
...
STATUS HDI_Init() {
  ...
  //uart_printf("%s: start-hdi_current_block=%d,hdi_current_page=%d\r\n",__func__,hdi_current_block,hdi_current_page);
...
STATUS FTL_HDI_hash_table_store(){  
  ...
  PAGE_OFF page_to_write=0;//写在第0页  
 
  ret=UBI_Write(block_to_write, page_to_write, &hdi_hash_table, NULL, FALSE);  
...
STATUS FTL_HDI_hash_table_restore(){
  ...
  UINT8 tem_hdi_hash_table[MPP_SIZE];
  
  LOG_BLOCK next_block = INVALID_BLOCK;
  
  ret=UBI_Read(block_to_write, page_to_write, &tem_hdi_hash_table, NULL);
  
  if(ret == STATUS_SUCCESS) {
   
   if (hdi_current_page == PAGE_PER_PHY_BLOCK) {    
    next_block = hdi_current_block ^ 1;//即逻辑II块4和5，交替使用 
    ret = UBI_Erase(next_block, next_block);//实际是直接物理擦除，不修改MAPII
    if (ret == STATUS_SUCCESS) {
      hdi_current_page = 0;
      hdi_current_block = next_block;
    }
  ...
  if (ret == STATUS_SUCCESS) {
    ret = UBI_Write(hdi_current_block, hdi_current_page, tem_hdi_hash_table, NULL,FALSE);
  }  
 } 
  return ret;
}
...
STATUS Truly_FTL_HDI_hash_table_restore(){  
  ...
  if (ret == STATUS_SUCCESS) {
    PM_NODE_SET_BLOCKPAGE(root_table.hdi_current_journal, hdi_current_block,hdi_current_page);
    hdi_current_page++;
  }
