...
STATUS UBI_Format() {
  ...
  //jsj增加,先不执行，不用每次都重建坏块表  
  ANCHOR_Format_Clean();//先擦除6个anchor 块
  /* try to read out only the anchor table first for bad block table */
  ret = ANCHOR_Init();
  //擦除6个anchor块后将导致每次上电都读anchor table失败
  //如果读anchor table失败，会重新扫描所有物理块，建立坏块表
  if (ret != STATUS_SUCCESS) {
    int bad_blk=0;
    PHY_BLOCK block;
    uart_printf("%s:Scan bad block start\r\n",__func__);
    /* detect bad block and build up the bad block table */
    //jsjpennn
    //todo:为了存code 从PEB 1开始擦，不擦PEB 0??
    for (block = 0; block < CFG_LOG_BLOCK_COUNT; block++) {     
      ...
      {
        bad_blk++;
        uart_printf("%s: Find a bad block: %d-Bad block at PEB %u\n", __func__, bad_blk, block);
        ANCHOR_LogBadBlock(block);//PEB2782是坏块
      }    
    }
  }  
  ...
  if (fmt_current_block != INVALID_BLOCK) {
    //已在INDEX_Format()中 保证Index block是好块    
    /* formatting index */    
        ...
        if (area == updating_area) {
          ...
          if (AREA_CheckUpdatePLR(updating_logical_block, updating_origin_block,updating_block_ec) == TRUE) {
            /* continue to update the area table */            
              ...
              uart_printf("**%s**: Warning-INDEX_Update_Commit() is called and may change MAPII\r\n", __func__); //jsjpenn注意：可能会修改MAP II            
...
STATUS UBI_Read(LOG_BLOCK block, PAGE_OFF page, void* buffer, SPARE spare) {
  ...
  if (block != INVALID_BLOCK && page != INVALID_PAGE) {
    ...
    phy_block = AREA_GetBlock(block); //由LEB得到PEB
    //uart_printf("Read PHY BLOCK: %d\n", phy_block);
...
STATUS UBI_Erase(LOG_BLOCK block, LOG_BLOCK die_index) {
  //uart_printf("UBI_ERASE log block %d\n", block);
  STATUS ret = STATUS_SUCCESS;
 
  //jsjpenn 
  PHY_BLOCK PEB_block;   
  PEB_block=AREA_GetBlock(block); //对应物理块号  
  //直接执行物理擦除，不修改MAPII    
  ret=MTD_Erase(PEB_block);
  if (ret !=STATUS_SUCCESS) {
    uart_printf("**%s**:Error at erase LEB %d  PEB %d,need to do bad block management\r\n", __func__,block,PEB_block);
    
    ANCHOR_LogBadBlock(PEB_block);
    //修改MAPII
    //从FBT中获得一个新的PEB，已在MTD_Erase()中保证返回的不是ValidPEB表中的块
    PHY_BLOCK new_block;
    ERASE_COUNT new_peb_ec;
    ret = INDEX_FreeBlock_Get(PEB_block, &new_block, &new_peb_ec);        
    if (ret == STATUS_SUCCESS) {
      INDEX_Update_AreaUpdate(block, new_block, new_peb_ec);
      ret = INDEX_Update_Commit();//更新MAPII=area table，index table和anchor table
      if(ret==STATUS_SUCCESS){
       uart_printf("*%s*:Bad block maagement done and Modify MAPII ok\r\n", __func__);
      }        
    }      
  }    
...
STATUS UBI_SWL() {
  ...
  if (max_physical_block != min_physical_block&& max_physical_block != INVALID_BLOCK
    ...
    ret = MTD_Erase(max_physical_block);//使用均衡中真正擦除FBT中返回的EC最大的块
