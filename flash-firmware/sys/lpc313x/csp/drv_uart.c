...
Boolean UartInit(Int32U IrqPrio) {
  ...
  pUart0RxFifo->PopIndx = pUart0RxFifo->PushIndx = pUart0TxFifo->PopIndx =
      pUart0TxFifo->PushIndx = 0;
...
void UartCalcDivider(Int32U Freq, Int32U Baud, pInt32U pDiv, pInt32U pAddDiv,
  ...
  for (MulTemp = 1; MulTemp < 16; ++MulTemp) {
    ...
    for (AddDivTemp = 1; AddDivTemp < 16; ++AddDivTemp) {
      ...
      Temp = (Baudrate > Baud) ? (Baudrate - Baud) : (Baud - Baudrate);
      if (Temp < Error) {
        Error = Temp;
        *pDiv = DivTemp;
        *pMul = MulTemp;
...
void UartSetLineCoding(UartLineCoding_t UartCoding) {
  ...
  // Check parameters
  if ((UartCoding.dwDTERate == 0)
      || (UartCoding.dwDTERate > UART_MAX_BAUD_RATE)) {
    return;
  }
